...
MCP3428::MCP3428(int8_t addr, i2c_inst_t * port, int bits, int gain): address(addr), i2c(port) {
    ...
    i2c_write_timeout_us(i2c, address, &configMask, 1, false, 500);
}
...
uint8_t MCP3428::scan() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i2c_write_timeout_us(i2c, address, &data, 1, false, 5000) != 1) return 0;
        uint8_t recv[3] = {0, 0, 0x80};
        while (recv[2] & 0x80) if (i2c_read_timeout_us(i2c, address, recv, 3, false, 5000) != 3) return 0;
        int16_t res;
